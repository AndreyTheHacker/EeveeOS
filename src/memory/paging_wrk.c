...
void * dumb_kmalloc(uint32_t size, int align) {
    void * ret = temp_mem;
    if(align && !IS_ALIGN(ret)) ret = (void*)PAGE_ALIGN(ret);
...
void allocate_page(page_directory_t * dir, uint32_t virtual_addr, uint32_t frame, int is_kernel, int is_writable) {
    ...
    if(!dir) {
        kern_printc("allocate_page: page directory is empty\n");
        return;
    }
...
void free_page(page_directory_t * dir, uint32_t virtual_addr, int free) {
    ...
    if(!dir->ref_tables[page_dir_idx]) {
        kern_printc("free_page: page dir entry does not exist\n");
        return;
    }
    page_table_t * table = dir->ref_tables[page_dir_idx];
    if(!table->pages[page_tbl_idx].present) {
        kern_printc("free_page: page table entry does not exist\n");
        return;
    }
...
void switch_page_directory(page_directory_t * page_dir, uint32_t phys) {
    uint32_t t;
    if(!phys)
        t = (uint32_t)virt2phys(TEMP_PAGE_DIRECTORY, page_dir);
    else
        t = (uint32_t)page_dir;
...
void page_fault_handler() {
    asm volatile("sti");
    _kern_printc("PARAM-PAM-PAM... EEVEE OS crash -> PAGE FAULT\nHALT!!!",10,0);
    for(;;){}
}
